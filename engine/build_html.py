...
function toggleHistory(tr) {
  ...
  if (!next || !next.classList.contains('history-row')) {
    ...
    var html = '<div class="history-wrap"><div class="history-summary">' + combined + '</div>'
      ...
      + '<th>Date</th><th style="text-align:center">Div</th><th>Line</th><th style="white-space:nowrap;width:1px;text-align:center">PIT</th><th style="white-space:nowrap;width:1px">R</th>'
    ...
    data.forEach(function(r) {
      ...
      var opHtml = opArr.length
        ? opArr.map(function(o,i){ return '<span style="white-space:nowrap">' + o + dqBadge(o) + (orArr[i] != null ? '<em class="hr"> ('+(orArr[i]||'NR')+')</em>' : '') + '</span>'; }).join(' /<wbr> ')
      ...
      html += '<tr' + rowCls + '>'
        + '<td style="white-space:nowrap">' + (r.dt||'').replace(/\/\d{4}$/, '') + '</td>'
        + '<td style="text-align:center">' + divPill + '</td>'
        + '<td>' + (r.ln||'') + '</td>'
        + '<td style="text-align:center">' + prHtml + '</td>'
