...
# ---------------------------------------------------------------------------
# Chat widget (floating, injected into every dashboard page)
# ---------------------------------------------------------------------------
...
#chat-fab {
  ...
  background: #1a5276; color: #fff; font-size: 22px;
  border: none; cursor: pointer; box-shadow: 0 3px 12px rgba(0,0,0,.25);
  display: flex; align-items: center; justify-content: center;
  transition: background .15s;
}
#chat-fab:hover { background: #154360; }
#chat-panel {
...
.chat-msg.user { align-self: flex-end; background: #1a5276; color: #fff;
                 border-bottom-right-radius: 3px; }
.chat-msg.asst { align-self: flex-start; background: #f0f4f8; color: #222;
                 border-bottom-left-radius: 3px; }
.chat-msg.err  { align-self: flex-start; background: #FCEBEB; color: #791F1F;
                 border-bottom-left-radius: 3px; }
...
#chat-input {
  flex: 1; padding: 7px 11px; border: 1px solid #ddd; border-radius: 20px;
  font-size: 12px; outline: none;
}
#chat-input:focus { border-color: #1a5276; }
#chat-send {
  padding: 7px 14px; background: #1a5276; color: #fff;
  border: none; border-radius: 20px; font-size: 12px;
  cursor: pointer; white-space: nowrap;
}
#chat-send:hover { background: #154360; }
#chat-send:disabled { background: #aaa; cursor: default; }
</style>

<button id="chat-fab" onclick="toggleChatPanel()" title="Ask the Tennis Assistant">🎾</button>
<div id="chat-panel">
  <div id="chat-hdr">
    <span>Tennis Assistant</span>
    <button id="chat-hdr-close" onclick="toggleChatPanel()">✕</button>
  </div>
  <div id="chat-msgs"></div>
  <div id="chat-input-row">
    <input id="chat-input" placeholder="Ask about lineups, players, standings…"
...
(function() {
  ...
  window.toggleChatPanel = function() {
    ...
    if (p.classList.contains('open')) {
      ...
      if (_msgs.length === 0) addBubble('asst', 'Hi! Ask me anything — player stories, lineup picks, matchup odds, standings math…');
    }
  };
  ...
  window.sendChat = function() {
    ...
    var typing = addBubble('asst typing', '…');
    ...
    .then(function(data) {
      typing.remove();
      if (data.error) {
        addBubble('err', '⚠ ' + data.error);
      } else {
        addBubble('asst', data.content);
